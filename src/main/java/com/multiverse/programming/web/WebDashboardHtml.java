...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Arial, sans-serif; }
    ...
    .server-conn-bar { display: flex; align-items: center; gap: 8px; background: #070a13; padding: 6px 12px; border-radius: 8px; border: 1px solid var(--card-border); font-size: 0.85rem; }
    .server-conn-bar input { background: transparent; border: none; color: #fff; font-size: 0.82rem; outline: none; width: 180px; }
    ...
    .status-dot.local { background: var(--primary); box-shadow: 0 0 10px var(--primary); }
    .status-dot.offline { background: var(--danger); box-shadow: 0 0 10px var(--danger); }
    
    /* Storage quota meter */
    .quota-container { display: flex; align-items: center; gap: 10px; background: #070a13; border: 1px solid var(--card-border); padding: 6px 14px; border-radius: 8px; font-size: 0.8rem; }
    .quota-bar-track { width: 90px; height: 7px; background: #1e293b; border-radius: 4px; overflow: hidden; }
    .quota-bar-fill { height: 100%; width: 0%; background: var(--primary); transition: width 0.3s, background 0.3s; }
    .quota-bar-fill.warning { background: var(--warning); }
    .quota-bar-fill.danger { background: var(--danger); }

    .help-btn { background: transparent; border: 1px solid var(--card-border); color: var(--text-muted); border-radius: 50%; width: 26px; height: 26px; display: inline-flex; align-items: center; justify-content: center; cursor: pointer; font-size: 0.8rem; }
    ...
    .container { max-width: 1440px; margin: 0 auto; padding: 24px; display: grid; grid-template-columns: 370px 1fr 390px; gap: 24px; flex: 1; width: 100%; }
    ...
    .bp-name { font-weight: 700; font-size: 0.95rem; color: #fff; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; max-width: 200px; }
    ...
    /* Turtle & Dispatch Panel */
    .turtle-list { display: flex; flex-direction: column; gap: 8px; max-height: 200px; overflow-y: auto; }
    .turtle-item { background: #0b1120; border: 1px solid var(--card-border); border-radius: 8px; padding: 10px; display: flex; justify-content: space-between; align-items: center; }
    ...
    input[type="text"], input[type="number"], select { background: #070a13; border: 1px solid var(--card-border); color: #fff; padding: 9px 12px; border-radius: 6px; font-size: 0.88rem; outline: none; }
...
<body>
  <header>
    ...
    <div class="header-controls">
      <!-- 15 MB Quota Widget -->
      <div class="quota-container" title="Storage Quota (Max 15 MB per player)">
        <span>💾 Quota:</span>
        <div class="quota-bar-track">
          <div class="quota-bar-fill" id="quotaFill"></div>
        </div>
        <span id="quotaText" style="font-weight: 700;">0.00 / 15.0 MB</span>
      </div>
      ...
      <button class="help-btn" onclick="openHelpModal()" title="Connection Guide">?</button>
    </div>
  </header>

  <!-- Help / Connection Guide Modal -->
  <div class="modal-overlay" id="helpModal" onclick="closeHelpModal(event)">
    <div class="modal-content" onclick="event.stopPropagation()">
      <div class="modal-header">
        <span class="modal-title">Blueprint Nexus Connection Guide</span>
      ...
      <div style="font-size:0.88rem; color:var(--text-muted); display:flex; flex-direction:column; gap:12px; line-height:1.5;">
        <p><strong style="color:#fff;">100% Client-Side Engine:</strong> This dashboard works completely offline! You can drop any <code style="color:var(--primary);">.litematic</code> or <code style="color:var(--primary);">.nbt</code> file right now to inspect layers, 3D voxels, and materials without needing any server running.</p>
        <p><strong style="color:#fff;">Connecting from GitHub Pages (HTTPS to HTTP):</strong> Browsers block unencrypted requests (<code style="color:var(--warning);">http://localhost:8080</code>) from HTTPS pages by default (Mixed Content). To connect live in-game:</p>
        <ul style="padding-left: 20px;">
          <li><strong>Option A:</strong> In Chrome/Edge, click the padlock/settings icon on the left of the URL bar &gt; <em>Site Settings</em> &gt; Set <strong>Insecure content</strong> to <em>Allow</em>.</li>
          <li><strong>Option B:</strong> Open the server's internal portal URL directly in your browser: <code style="color:var(--primary);">http://localhost:8080</code> (served by the plugin itself with no mixed content restrictions!).</li>
          <li><strong>Option C:</strong> Use an HTTPS tunnel like ngrok or Cloudflare Tunnel (e.g. <code style="color:var(--primary);">https://your-domain.ngrok-free.app</code>).</li>
        </ul>
        <p><strong style="color:#fff;">Player Quota (15 MB):</strong> Each player has a dedicated 15 MB storage quota. Malicious blocks (Command Blocks, Bedrock, Barriers) are automatically filtered for server safety.</p>
      </div>
  ...
  <main class="container">
    ...
    <div class="card">
      ...
      <div class="dropzone" id="dropzone" onclick="document.getElementById('fileInput').click()">
        ...
        <div class="dropzone-sub">Parsed client-side &bull; Zero upload wait</div>
      </div>
      <input type="file" id="fileInput" accept=".litematic,.nbt" onchange="handleFileSelect(event)">

      <!-- Search & Filter -->
      <div class="search-bar">
        <input type="text" class="search-input" id="searchInput" placeholder="Search blueprints or blocks..." oninput="filterBlueprints(this.value)">
      </div>
    ...
    <div class="card">
      <div class="card-title">
        ...
        <button class="btn btn-sm btn-ghost" onclick="fetchTurtles()">⟳ Refresh</button>
      </div>
      ...
      <div class="form-group">
        <div style="display:flex; justify-content:space-between; align-items:center;">
          ...
          <a href="#" style="font-size:0.75rem; color:var(--primary); text-decoration:none;" onclick="useTurtleCoords()">Use Turtle Coords</a>
        </div>
      ...
      <!-- Live Job Progress -->
      <div id="liveJobCard" style="display:none; background:#070a13; border: 1px solid var(--card-border); border-radius: 8px; padding: 14px; margin-top: 10px;">
        ...
        <div style="display:flex; gap: 8px;">
          <button class="btn btn-sm btn-ghost" style="flex:1;" onclick="togglePauseJob()">Pause / Resume</button>
  ...
  <script>
    ...
    const MAX_QUOTA_BYTES = 15 * 1024 * 1024; // 15 MB per player
    let usedQuotaBytes = 0;
    ...
    function updateServerStatusDot(mode) {
      const dot = document.getElementById('serverStatusDot');
      dot.className = 'status-dot ' + mode; // 'online', 'local', 'offline'
    }
    ...
    // =========================================================================
    // LOCAL STORAGE & 15 MB QUOTA MANAGEMENT
    // =========================================================================
    function updateQuotaDisplay() {
      const usedMb = usedQuotaBytes / (1024 * 1024);
      const percent = Math.min(100, (usedQuotaBytes / MAX_QUOTA_BYTES) * 100);
      
      const fill = document.getElementById('quotaFill');
      fill.style.width = percent.toFixed(1) + '%';
      fill.className = 'quota-bar-fill' + (percent > 85 ? ' danger' : percent > 60 ? ' warning' : '');

      document.getElementById('quotaText').textContent = `${usedMb.toFixed(2)} / 15.0 MB`;
    }
      ...
      try {
        ...
        if (raw) {
          ...
          localList.forEach(bp => {
            usedQuotaBytes += (bp.fileSizeBytes || (bp.totalBlocks * 4));
    ...
    function saveToLocalLibrary(bp, fileSizeBytes) {
      if (usedQuotaBytes + fileSizeBytes > MAX_QUOTA_BYTES) {
        showToast('⚠️ Storage quota exceeded! Max 15 MB per player.', true);
        return false;
      }
      bp.fileSizeBytes = fileSizeBytes;
      bp.isLocal = true;
      bp.owner = bp.owner || 'You';

      // Remove duplicate if exists
      blueprints = blueprints.filter(b => b.id !== bp.id);
      blueprints.unshift(bp);

      usedQuotaBytes += fileSizeBytes;
      updateQuotaDisplay();
      ...
      try {
        ...
        console.warn('LocalStorage limit reached for full structure, caching metadata:', e);
      }
      return true;
    }
    ...
    function deleteBlueprint(id) {
      ...
      if (bp.isLocal) {
        usedQuotaBytes = Math.max(0, usedQuotaBytes - (bp.fileSizeBytes || 0));
        blueprints = blueprints.filter(b => b.id !== id);
        const localsOnly = blueprints.filter(b => b.isLocal);
        localStorage.setItem('mp_user_blueprints', JSON.stringify(localsOnly));
        updateQuotaDisplay();
        showToast('🗑️ Deleted blueprint: ' + bp.name);
      }

      // Try server delete if connected
      fetch(getApiBase() + '/api/delete', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ blueprintId: id, player: 'WebPlayer' })
      }).catch(() => {});
      ...
      if (selectedBlueprint && selectedBlueprint.id === id) {
        ...
        document.getElementById('matTableBody').innerHTML = '<tr><td colspan="3" style="text-align:center; padding:20px; color:var(--text-muted);">Select a blueprint to calculate materials</td></tr>';
        clearCanvas();
      }
    ...
    async function loadCommunityCatalog() {
      // Preload user's Nether Portal sample structure
      const sample = {
        id: 'BP-NETHER-PORTAL',
    ...
    // =========================================================================
    // CLIENT-SIDE ZERO-DEPENDENCY NBT & LITEMATICA PARSER
    // =========================================================================
    async function decompressGzip(arrayBuffer) {
      ...
      return arrayBuffer; // Not compressed or uncompressed NBT
    }
        ...
        for (let i = 0; i < total; i++) {
          ...
          if (pIdx >= 0 && pIdx < palette.length) {
            ...
            if (mat !== 'AIR' && mat !== 'CAVE_AIR' && mat !== 'VOID_AIR') {
              ...
              const x = i % rx;
              blocks.push({ x, y, z, blockState: mat, material: mat });
      ...
      if (root.blocks) {
        root.blocks.forEach(b => {
          ...
          if (mat !== 'AIR' && mat !== 'CAVE_AIR' && mat !== 'VOID_AIR' && b.pos && b.pos.length >= 3) {
            const block = { x: b.pos[0], y: b.pos[1], z: b.pos[2], blockState: mat, material: mat };
            blocks.push(block);
            materials[mat] = (materials[mat] || 0) + 1;
          }
        });
    ...
    async function processBlueprintFile(file) {
      ...
      showToast('⚡ Processing ' + file.name + ' client-side…');
      try {
        ...
        // Save to browser quota
        const saved = saveToLocalLibrary(parsedBp, file.size);
        if (!saved) return;
        ...
        // Also attempt background sync to server if available
        syncWithServerIfPossible(file.name, rawBuffer);
      } catch (err) {
        console.error(err);
    ...
    async function syncWithServerIfPossible(filename, rawBuffer) {
      try {
        ...
        const res = await fetch(getApiBase() + '/api/upload', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ filename, data: base64Data, player: 'WebPlayer' })
        });
        const data = await res.json();
        if (data.ok) {
          updateServerStatusDot('online');
          showToast('☁️ Synced to Minecraft server!');
        }
      } catch (err) {
        // Safe fallback - offline/client mode
        updateServerStatusDot('local');
      }
    }
    ...
    function syncLibrary() {
      ...
      // Fetch server blueprints
      fetch(getApiBase() + '/api/blueprints')
        .then(res => res.json())
        .then(serverBps => {
          updateServerStatusDot('online');
          serverBps.forEach(sbp => {
            if (!blueprints.some(b => b.id === sbp.id)) {
              blueprints.push(sbp);
            }
          });
          renderBlueprintList();
    ...
    function renderBlueprintList(filterText = '') {
      ...
      const filtered = blueprints.filter(bp => {
        ...
        return bp.name.toLowerCase().includes(q) || (bp.format && bp.format.toLowerCase().includes(q));
      });
      ...
      list.innerHTML = filtered.map(bp => `
        <div class="bp-item ${selectedBlueprint && selectedBlueprint.id === bp.id ? 'selected' : ''}" onclick="selectBlueprint('${bp.id}')">
          ...
          <div class="bp-meta">
            <span>📐 ${bp.sizeX}×${bp.sizeY}×${bp.sizeZ} &bull; 🧱 ${bp.totalBlocks.toLocaleString()}</span>
            <span style="color:var(--text-dim);">${escapeHtml(bp.owner || bp.author || '')}</span>
          </div>
          <div class="bp-actions" onclick="event.stopPropagation()">
            ${bp.isLocal ? `<button class="btn btn-sm btn-ghost" style="color:var(--danger);" onclick="deleteBlueprint('${bp.id}')">Delete</button>` : ''}
          </div>
        </div>
      `).join('');
    }
    ...
    async function selectBlueprint(id) {
      ...
      // If sample without full blocks, load sample file
      ...
      // If server blueprint without blocks, fetch from /api/blueprints/:id
      if (!bp.blocks || bp.blocks.length === 0) {
        try {
          const res = await fetch(getApiBase() + '/api/blueprints/' + id);
          const full = await res.json();
          bp.blocks = full.blocks;
          bp.materialCounts = full.materialCounts;
        } catch (e) {
          console.warn('Could not fetch server blueprint blocks:', e);
        }
      }
      ...
      document.getElementById('visualizerDims').textContent = `${bp.sizeX} × ${bp.sizeY} × ${bp.sizeZ} (${bp.totalBlocks.toLocaleString()} blocks) &bull; Author: ${bp.author || 'Unknown'}`;
    ...
    function render3DIsometric(ctx, w, h, maxLayer, sx, sy, sz) {
      ...
      // screenY = originY + (x + z) * tileHeight / 2 - y * blockHeight
      ...
      // Sort blocks for correct isometric depth (back to front: y, then x+z)
      blocks.sort((a, b) => {
        if (a.y !== b.y) return a.y - b.y;
        return (a.x + a.z) - (b.x + b.z);
      });

      // Render voxel isometric cubes
      blocks.forEach(b => {
        let x = b.x, z = b.z;
        if (currentRotation === 90) { const tx = x; x = sz - 1 - z; z = tx; }
        else if (currentRotation === 180) { x = sx - 1 - x; z = sz - 1 - z; }
    ...
    function fetchTurtles() {
      fetch(getApiBase() + '/api/turtles')
        ...
        .catch(() => {
          // Keep local status if server is unreachable
          updateServerStatusDot(blueprints.some(b => b.isLocal) ? 'local' : 'offline');
        });
    }
      ...
      list.innerHTML = turtles.map(t => `
        <div class="turtle-item">
          <div>
            ...
            <div style="font-size:0.75rem; color:var(--text-muted);">Facing: ${t.facing} &bull; Fuel: ${t.fuel}</div>
          </div>
    ...
    function updateDispatchDropdowns() {
      ...
      bpSelect.innerHTML = '<option value="">-- Choose Blueprint --</option>' +
        blueprints.map(b => `<option value="${b.id}" ${b.id === curBp ? 'selected' : ''}>${escapeHtml(b.name)} (${b.totalBlocks.toLocaleString()} blocks)</option>`).join('');
    }

    function onDispatchBpSelect(bpId) { if (bpId) selectBlueprint(bpId); }
    ...
    function startDispatchBuild() {
      ...
      .catch(err => showToast('Connection error: ' + err, true));
    }
